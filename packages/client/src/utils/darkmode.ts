...
export const getDarkStyle = (selector?: string | boolean): string => {
  const darkStyle = isString(selector)
    ...
    : selector === true
      ? `:root${DARK_VARIABLES}`
      : '';
  return `:root{--waline-badge-light:initial;--waline-badge-dark: }${darkStyle}`;
};
